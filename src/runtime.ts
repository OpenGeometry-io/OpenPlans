...
import { type LineOptions as OpenPlansLineOptions, LinePrimitive } from './primitives/line';
...
import {
  exportIfc as exportSemanticIfc,
  generateDocument as generateSemanticDocument,
  PlanDocument,
  type DoorElement,
  type Point3D,
...
export class OpenPlans {
  ...
  private semanticDocument: PlanDocument = new PlanDocument();
  private openGeometryWasmURL?: string;
  ...
  get document() {
    return this.semanticDocument;
  }

  createPlanDocument(structure?: {
    projectName?: string;
    siteName?: string;
    buildingName?: string;
    storeyName?: string;
  }) {
    this.semanticDocument = new PlanDocument(structure);
    return this.semanticDocument;
  }
  ...
  async setupOpenGeometry(wasmURL?: string) {
    this.openGeometryWasmURL = wasmURL ?? this.openGeometryWasmURL;
    await OpenGeometry.create({ wasmURL: this.openGeometryWasmURL });
  ...
  // Primitives
  line(config?: ILineOptions | OpenPlansLineOptions) {
    const lineConfig =
      config && "startPoint" in config
        ? config
        : config
          ? {
              ogid: config.ogid,
              startPoint: [config.start.x, config.start.y, config.start.z],
              endPoint: [config.end.x, config.end.y, config.end.z],
              color: config.color,
              fatLines: config.fatLines ?? true,
              width: config.width ?? 4,
            }
          : undefined;
    const line = new LinePrimitive(lineConfig);
  ...
  convertImpleniaToOGFormat(sourceJson: any) {
    return convertToOGFormat(sourceJson);
  }

  generateDocument() {
    this.refreshSemanticDocument();
    return generateSemanticDocument(this.semanticDocument);
  }

  async exportIfc(options?: SemanticIfcExportOptions) {
    this.refreshSemanticDocument();
    return exportSemanticIfc(this.semanticDocument, {
      ...options,
      wasmURL: this.openGeometryWasmURL,
    });
  }

  private refreshSemanticDocument() {
    const nextDocument = new PlanDocument(this.semanticDocument.structure);

    const walls = this.ogElements.filter((entity) => entity instanceof Wall2D) as Wall2D[];
    for (const wall of walls) {
      nextDocument.upsertWall(this.toSemanticWall(wall));
    }

    const doors = this.ogElements.filter(
      (entity) =>
        entity instanceof Door2D ||
        entity instanceof DoubleDoor2D ||
        entity instanceof BaseDoor,
    ) as Array<Door2D | DoubleDoor2D | BaseDoor>;
    for (const door of doors) {
      nextDocument.upsertDoor(this.toSemanticDoor(door, nextDocument));
    }

    const windows = this.ogElements.filter(
      (entity) =>
        entity instanceof Window2D ||
        entity instanceof DoubleWindow2D ||
        entity instanceof BaseSingleWindow ||
        entity instanceof BaseDoubleWindow,
    ) as Array<Window2D | DoubleWindow2D | BaseSingleWindow | BaseDoubleWindow>;
    for (const window of windows) {
      nextDocument.upsertWindow(this.toSemanticWindow(window, nextDocument));
    }

    this.semanticDocument = nextDocument;
    return this.semanticDocument;
  }

  private toSemanticWall(wall: Wall2D): Omit<WallElement, "kind"> & { kind: "WALL" } {
    return {
      id: wall.propertySet.ogid ?? wall.ogid,
      kind: "WALL",
      labelName: wall.labelName,
      points: wall.propertySet.points.map((point) => ({
        x: point.x + wall.position.x,
        y: point.y + wall.position.y,
        z: point.z + wall.position.z,
      })),
  ...
  private toWorldPoint(
    ...
    if (Array.isArray(point)) {
      return {
        x: point[0] + elementPosition.x,
        y: point[1] + elementPosition.y,
        z: point[2] + elementPosition.z,
      };
    }
    ...
    return {
      ...
      z: point.z + elementPosition.z,
    };
  }

  private toSemanticDoor(
    door: Door2D | DoubleDoor2D | BaseDoor,
    document: PlanDocument,
  ): Omit<DoorElement, "kind"> & { kind: "DOOR" } {
    const worldPosition = this.toWorldPoint(door.propertySet.doorPosition, door.position);

    if (door instanceof BaseDoor) {
      return {
        id: door.propertySet.ogid ?? door.ogid,
        kind: "DOOR",
        labelName: door.labelName,
        hostWallId: this.findNearestWallId(worldPosition, document),
        doorPosition: worldPosition,
        doorWidth: door.propertySet.dimensions.length,
        doorHeight: door.propertySet.doorHeight,
        doorThickness: door.propertySet.doorThickness,
        frameThickness: door.propertySet.frameThickness,
        doorColor: door.propertySet.doorColor,
        frameColor: door.propertySet.frameColor,
        doorMaterial: String(door.propertySet.doorType ?? "generic"),
        appearance: {
          doorColor: door.propertySet.doorColor,
          frameColor: door.propertySet.frameColor,
          materialLabel: String(door.propertySet.doorType ?? "generic"),
        },
      };
    }

    if (door instanceof DoubleDoor2D) {
      const totalDoorWidth =
        door.propertySet.doorDimensions.width * 2 + door.propertySet.mullionWidth;
      ...
      return {
        ...
        hostWallId: this.findNearestWallId(worldPosition, document),
        doorPosition: worldPosition,
        doorWidth: totalDoorWidth,
        doorHeight: door.propertySet.doorDimensions.height ?? 2.1,
        doorThickness: door.propertySet.doorDimensions.thickness,
        frameThickness: door.propertySet.frameDimensions.thickness,
        doorColor: door.propertySet.doorColor,
        frameColor: door.propertySet.frameColor,
        doorMaterial: String(door.propertySet.doorMaterial),
        appearance: {
          doorColor: door.propertySet.doorColor,
          frameColor: door.propertySet.frameColor,
          materialLabel: String(door.propertySet.doorMaterial),
        },
      };
    }
    ...
    return {
      ...
      hostWallId: this.findNearestWallId(worldPosition, document),
      doorPosition: worldPosition,
      doorWidth: door.propertySet.doorDimensions.width,
      doorHeight: door.propertySet.doorDimensions.height ?? 2.1,
      doorThickness: door.propertySet.doorDimensions.thickness,
      frameThickness: door.propertySet.frameDimensions.thickness,
      doorColor: door.propertySet.doorColor,
      frameColor: door.propertySet.frameColor,
      doorMaterial: door.propertySet.doorMaterial,
      appearance: {
        doorColor: door.propertySet.doorColor,
        frameColor: door.propertySet.frameColor,
        materialLabel: door.propertySet.doorMaterial,
      },
    };
  }

  private toSemanticWindow(
    window: Window2D | DoubleWindow2D | BaseSingleWindow | BaseDoubleWindow,
    document: PlanDocument,
  ): Omit<WindowElement, "kind"> & { kind: "WINDOW" } {
    const worldPosition = this.toWorldPoint(window.propertySet.windowPosition, window.position);

    if (window instanceof BaseSingleWindow) {
      return {
        id: window.propertySet.ogid ?? window.ogid,
        kind: "WINDOW",
        labelName: window.labelName,
        hostWallId: this.findNearestWallId(worldPosition, document),
        windowPosition: worldPosition,
        windowWidth: window.propertySet.dimensions.length,
        windowHeight: window.propertySet.windowHeight,
        windowThickness: window.propertySet.windowThickness,
        sillHeight: window.propertySet.sillHeight,
        frameColor: window.propertySet.frameColor,
        glassColor: window.propertySet.windowColor,
        windowMaterial: String(window.propertySet.windowType ?? "glass"),
        appearance: {
          frameColor: window.propertySet.frameColor,
          glassColor: window.propertySet.windowColor,
          materialLabel: String(window.propertySet.windowType ?? "glass"),
        },
      };
    }

    if (window instanceof BaseDoubleWindow) {
      return {
        id: window.propertySet.ogid ?? window.ogid,
        kind: "WINDOW",
        labelName: window.labelName,
    ...
    if (window instanceof DoubleWindow2D) {
      const totalWindowWidth =
        window.propertySet.frameDimensions.width * 2 +
        window.propertySet.windowDimensions.width * 2 +
        window.propertySet.mullionWidth;
      ...
      return {
        ...
        hostWallId: this.findNearestWallId(worldPosition, document),
        windowPosition: worldPosition,
        windowWidth: totalWindowWidth,
        windowHeight: 1.2,
        windowThickness: window.propertySet.windowDimensions.thickness,
        sillHeight: 0.9,
        frameColor: window.propertySet.frameColor,
        glassColor: window.propertySet.glassColor,
    ...
    return {
      ...
      hostWallId: this.findNearestWallId(worldPosition, document),
      windowPosition: worldPosition,
      windowWidth: window.propertySet.windowDimensions.width,
      windowHeight: 1.2,
      windowThickness: window.propertySet.windowDimensions.thickness,
      sillHeight: 0.9,
      frameColor: window.propertySet.frameColor,
  ...
  private findNearestWallId(point: Point3D, document: PlanDocument) {
